...
from gui.views.notification_list_view import NotificationListView
...
class AppShell(ctk.CTkFrame):
    ...
    def _build_sidebar(self) -> None:
        ...
        nav_items = [
            "Órdenes de Venta", "Embarques", "Notificaciones",
            "Reportes Ejecutivos", "Usuarios y Roles",
        ]
    ...
    def _navigate(self, item: str) -> None:
        ...
        elif item == "Notificaciones":
            controller = NotificationController(session=session)
            NotificationListView(self.content, controller=controller).grid(
                row=0, column=0, sticky="nswe", pady=10
            )
        elif item == "Reportes Ejecutivos":
            ReportsView(self.content, session=session).grid(
                row=0, column=0, sticky="nswe", pady=10
            )
